				...
				<div class="modal-dialog" role="document">
					...
					<div class="modal-body">
						<form action="functions/functions.php" method="POST" id="formJobTitle">
							...
							<input type="hidden" name="actionType" value="addLeaveTitle">
						</form>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
						<button type="button" class="btn btn-primary" id="submitJobTitle">Accept</button>
						<button type="button" class="btn btn-danger" id="deleteLeaveTitle">Delete</button>
					</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			var jobTitlesTable = $('#jobTitles').DataTable( {
				...
				"order": [[ 1, "desc" ]],
				"ajax": {
					"url": 'functions/tables/leave-type.php',
					"type": "POST",
			...
			$('#jobTitles').on('click', '.triggerActions', function () {
				...
				$('#formJobTitle input[name ="actionType"]').val('updateLeaveTitle');
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#exampleModal').on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				modal.find('.modal-title').text('Add new Leave Type/Name');
				...
				var actionType = $('#formJobTitle input[name ="actionType"]').val('addLeaveTitle');
				//Pass the selected records value
				var record = $("<input>").attr("type", "hidden").attr("name", "rec").val(null);
				$('#formJobTitle').append(record);

				$('#view-spec').hide();
				$('#deleteLeaveTitle').hide();
				...
				$(function () {
					$('body').on('click', '#deleteLeaveTitle', function (e) {
						$.confirm({
							title: 'Delete',
							content: 'Are you sure you would like to delete this record?',
							buttons: {
								confirm: function () {
									$('#formJobTitle input[name ="actionType"]').val('deleteLeaveTitle');
									$('#formJobTitle').submit();
									$('#myModal').modal('hide');
								},
								cancel: function () {
